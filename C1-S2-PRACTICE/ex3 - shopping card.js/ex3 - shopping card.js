...
function getCartTotalAmount() {
  ...
  for (const item of SHOPPING_CART) {
    ...
    if (product) {
      result = result + product.price * item.quantity;
    }
  }
  return result;
}

//  Vòng lặp for of được sử dụng để duyệt qua từng phần tử trong mảng SHOPPING_CART. 
//Ở mỗi lần lặp, phần tử hiện tại được gán cho biến item.
